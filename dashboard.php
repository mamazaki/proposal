<?php
session_start();
require 'config.php';

if (!isset($_SESSION['user_id'])) {
    header("Location: login.php");
    exit;
}

// ------------------------------------------------------------------
// 1. รับค่าตัวกรอง (Filters) จาก URL
// ------------------------------------------------------------------
$stmt_mt = $pdo->query("SELECT id, title FROM main_topics WHERE is_active = 1 ORDER BY id ASC");
$main_topics = $stmt_mt->fetchAll();

$selected_main_topic_id = $_GET['main_topic_id'] ?? ($main_topics[0]['id'] ?? 0);
$selected_sub_group_id  = $_GET['sub_group_id'] ?? '';
$selected_agency        = $_GET['agency'] ?? '';

// ------------------------------------------------------------------
// 2. ระบบประมวลผลการลบข้อมูล (Soft Delete)
// ------------------------------------------------------------------
if (isset($_GET['del_id'])) {
    $del_id = $_GET['del_id'];
    $stmt_check = $pdo->prepare("SELECT user_id, first_name, last_name FROM committee_members WHERE id = ?");
    $stmt_check->execute([$del_id]);
    $record = $stmt_check->fetch();
    
    if ($record && ($_SESSION['role'] === 'admin' || $_SESSION['user_id'] == $record['user_id'])) {
        try {
            $pdo->beginTransaction();
            $stmt_del = $pdo->prepare("UPDATE committee_members SET is_active = 0 WHERE id = ?");
            $stmt_del->execute([$del_id]);
            systemLog($pdo, 'DELETE (Soft)', 'committee_members', $del_id, ['deleted_name' => $record['first_name'].' '.$record['last_name']]);
            $pdo->commit();
            // ลบเสร็จให้กลับมาพร้อม Filter เดิม
            echo "<script>alert('ลบรายชื่อเรียบร้อยแล้ว'); window.location.href='dashboard.php?main_topic_id={$selected_main_topic_id}&sub_group_id={$selected_sub_group_id}&agency=".urlencode($selected_agency)."';</script>";
            exit;
        } catch (Exception $e) {
            $pdo->rollBack();
            die("Error: " . $e->getMessage());
        }
    }
}

// ------------------------------------------------------------------
// 3. เตรียมข้อมูลสำหรับทำ Dropdown ตัวกรอง (ค้นหาเฉพาะที่ตรงกับ Main Topic)
// ------------------------------------------------------------------
// ดึงประเด็นย่อย
$stmt_filter_sg = $pdo->prepare("SELECT id, title FROM sub_groups WHERE main_topic_id = ? ORDER BY sort_order ASC");
$stmt_filter_sg->execute([$selected_main_topic_id]);
$filter_subgroups = $stmt_filter_sg->fetchAll();

// ดึงรายชื่อหน่วยงานที่มีการบันทึกแล้วแบบไม่ซ้ำ (DISTINCT) เพื่อทำ Dropdown
$stmt_filter_agency = $pdo->prepare("SELECT DISTINCT cm.agency 
                                     FROM committee_members cm 
                                     JOIN sub_groups sg ON cm.sub_group_id = sg.id 
                                     WHERE cm.is_active = 1 AND sg.main_topic_id = ? 
                                     ORDER BY cm.agency ASC");
$stmt_filter_agency->execute([$selected_main_topic_id]);
$filter_agencies = $stmt_filter_agency->fetchAll();

// ------------------------------------------------------------------
// 4. อัปเดต Query ข้อมูล (Dynamic Query Building)
// ------------------------------------------------------------------

// 4.1 สรุปจำนวนคณะทำงานแต่ละประเด็นย่อย (อิงตาม Main Topic)
$sql_summary = "SELECT sg.title, COUNT(cm.id) as total 
                FROM sub_groups sg 
                LEFT JOIN committee_members cm ON sg.id = cm.sub_group_id AND cm.is_active = 1
                WHERE sg.main_topic_id = ?
                GROUP BY sg.id ORDER BY sg.sort_order";
$stmt_summary = $pdo->prepare($sql_summary);
$stmt_summary->execute([$selected_main_topic_id]);
$summary_data = $stmt_summary->fetchAll();

// 4.2 สร้าง Dynamic Query สำหรับดึงรายชื่อทั้งหมด (ตาม Filter ที่เลือก)
$sql_all = "SELECT cm.*, sg.title as subgroup_title, mt.title as main_topic_title, u.full_name as author_name 
            FROM committee_members cm
            JOIN sub_groups sg ON cm.sub_group_id = sg.id
            JOIN main_topics mt ON sg.main_topic_id = mt.id
            LEFT JOIN users u ON cm.user_id = u.id
            WHERE cm.is_active = 1 AND mt.id = ?";
$params = [$selected_main_topic_id];

// ถ้ามีการเลือกประเด็น ให้ต่อ SQL และเพิ่ม parameter
if ($selected_sub_group_id !== '') {
    $sql_all .= " AND sg.id = ?";
    $params[] = $selected_sub_group_id;
}

// ถ้ามีการเลือกหน่วยงาน ให้ต่อ SQL และเพิ่ม parameter
if ($selected_agency !== '') {
    $sql_all .= " AND cm.agency = ?";
    $params[] = $selected_agency;
}

$sql_all .= " ORDER BY mt.id ASC, sg.sort_order ASC, cm.sort_order ASC, cm.id ASC";
$stmt_all = $pdo->prepare($sql_all);
$stmt_all->execute($params);
$all_members = $stmt_all->fetchAll();
?>
<!DOCTYPE html>
<html lang="th">
<head>
    <meta charset="UTF-8">
    <title>Dashboard</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="bg-light d-flex flex-column min-vh-100">
    
    <?php include 'navbar.php'; ?>

    <div class="container-fluid px-4 flex-grow-1">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3>Dashboard สรุปการเสนอชื่อ</h3>
            <div>
                <button type="button" class="btn btn-success me-2" onclick="exportToExcel()">
                    📥 ดาวน์โหลด Excel (.xlsx)
                </button>
                <a href="index.php" class="btn btn-primary">+ เสนอชื่อเพิ่ม</a>
            </div>
        </div>

        <div class="card shadow-sm mb-4 border-primary">
            <div class="card-header bg-primary text-white fw-bold">
                🔍 กรองข้อมูลรายชื่อ
            </div>
            <div class="card-body bg-white py-3">
                <form method="GET" action="dashboard.php" id="filterForm">
                    <div class="row g-3 align-items-end">
                        <div class="col-md-4">
                            <label class="form-label fw-bold text-primary">📂 หัวข้อเรื่องหลัก</label>
                            <select name="main_topic_id" class="form-select" onchange="resetSubFiltersAndSubmit()">
                                <?php foreach($main_topics as $mt): ?>
                                    <option value="<?= $mt['id'] ?>" <?= ($mt['id'] == $selected_main_topic_id) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($mt['title']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        
                        <div class="col-md-4">
                            <label class="form-label fw-bold text-success">📑 ประเด็น / คณะทำงานย่อย</label>
                            <select name="sub_group_id" id="subGroupFilter" class="form-select">
                                <option value="">-- แสดงทุกประเด็น --</option>
                                <?php foreach($filter_subgroups as $sg): ?>
                                    <option value="<?= $sg['id'] ?>" <?= ($sg['id'] == $selected_sub_group_id) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($sg['title']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        
                        <div class="col-md-3">
                            <label class="form-label fw-bold text-danger">🏢 หน่วยงานต้นสังกัด</label>
                            <select name="agency" id="agencyFilter" class="form-select">
                                <option value="">-- แสดงทุกหน่วยงาน --</option>
                                <?php foreach($filter_agencies as $ag): ?>
                                    <option value="<?= htmlspecialchars($ag['agency']) ?>" <?= ($ag['agency'] === $selected_agency) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($ag['agency']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        
                        <div class="col-md-1 text-end">
                            <button type="submit" class="btn btn-primary w-100 fw-bold">ค้นหา</button>
                        </div>
                    </div>
                    <?php if($selected_sub_group_id !== '' || $selected_agency !== ''): ?>
                        <div class="mt-2 text-end">
                            <a href="dashboard.php?main_topic_id=<?= $selected_main_topic_id ?>" class="text-danger text-decoration-none"><small>❌ ล้างตัวกรองประเด็นและหน่วยงาน</small></a>
                        </div>
                    <?php endif; ?>
                </form>
            </div>
        </div>
        
        <div class="row">
            <?php foreach($summary_data as $row): ?>
            <div class="col-md-3 mb-3">
                <div class="card shadow-sm h-100 border-0 border-start border-4 border-success">
                    <div class="card-body">
                        <p class="card-title text-muted mb-1" style="font-size: 0.85rem; height: 40px; overflow: hidden;"><?= htmlspecialchars($row['title']) ?></p>
                        <h3 class="text-success mb-0"><?= $row['total'] ?> <small class="text-muted fs-6">คน</small></h3>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
        </div>

        <hr>
        
        <div class="d-flex justify-content-between align-items-center mb-2">
            <h4 class="mb-0">รายชื่อทั้งหมด</h4>
            <span class="badge bg-info text-dark">พบข้อมูล <?= count($all_members) ?> รายการ</span>
        </div>
        
        <div class="table-responsive bg-white shadow-sm mb-4">
            <table class="table table-bordered table-striped table-hover align-middle mb-0">
                <thead class="table-dark">
                    <tr>
                        <th width="20%">คณะทำงาน/ประเด็น</th>
                        <th width="15%">ตำแหน่งในคณะ</th>
                        <th width="15%">ชื่อ - นามสกุล</th>
                        <th width="18%">ตำแหน่ง/สังกัด</th>
                        <th width="12%">เบอร์โทร</th>
                        <th width="10%">ผู้เพิ่มข้อมูล</th>
                        <th width="10%" class="text-center">จัดการ</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if(count($all_members) > 0): ?>
                        <?php foreach($all_members as $row): ?>
                        <tr>
                            <td><small><?= htmlspecialchars($row['subgroup_title']) ?></small></td>
                            <td>
                                <?= htmlspecialchars($row['wg_position']) ?>
                                <br><small class="text-muted">(ลำดับ: <?= $row['sort_order'] ?>)</small>
                            </td>
                            <td>
                                <?= htmlspecialchars($row['prefix']) ?>&nbsp;<?= htmlspecialchars($row['first_name']) ?>&nbsp;&nbsp;<?= htmlspecialchars($row['last_name']) ?>
                            </td>
                            <td>
                                <?= htmlspecialchars($row['job_position']) ?>
                                <br><strong class="text-primary"><small><?= htmlspecialchars($row['agency']) ?></small></strong>
                            </td>
                            <td><?= htmlspecialchars($row['phone_number']) ?></td>
                            <td>
                                <span class="badge bg-secondary">
                                    <?= str_replace(' ', '&nbsp;', htmlspecialchars($row['author_name'] ?? $row['username'] ?? 'System')) ?>
                                </span>
                            </td>
                            <td class="text-center">
                                <?php if($_SESSION['role'] === 'admin' || $_SESSION['user_id'] == $row['user_id']): ?>
                                    <div class="btn-group" role="group">
                                        <a href="edit.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">แก้ไข</a>
                                        <a href="?del_id=<?= $row['id'] ?>&main_topic_id=<?= $selected_main_topic_id ?>&sub_group_id=<?= $selected_sub_group_id ?>&agency=<?= urlencode($selected_agency) ?>" class="btn btn-sm btn-danger" onclick="return confirm('คุณแน่ใจหรือไม่ว่าต้องการลบรายชื่อ: <?= htmlspecialchars($row['first_name'].' '.$row['last_name']) ?> ?');">ลบ</a>
                                    </div>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="7" class="text-center py-4 text-danger fw-bold">ไม่พบข้อมูลตามเงื่อนไขที่ค้นหา</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    </div>

    <?php include 'footer.php'; ?>

    <script src="https://cdn.sheetjs.com/xlsx-0.20.0/package/dist/xlsx.full.min.js"></script>
    <script>
    // สคริปต์เมื่อเปลี่ยนหัวข้อหลัก ให้ล้างค่า Filter ย่อยทิ้งก่อนแล้วค่อย submit เพื่อไม่ให้ติดค่าขยะ
    function resetSubFiltersAndSubmit() {
        document.getElementById('subGroupFilter').value = '';
        document.getElementById('agencyFilter').value = '';
        document.getElementById('filterForm').submit();
    }

    function exportToExcel() {
        let data = [
            ["ลำดับ", "คณะทำงาน/ประเด็น", "คำนำหน้า", "ชื่อ", "นามสกุล", "ตำแหน่งหน้าที่", "สังกัด", "ตำแหน่งในคณะ", "เบอร์โทร", "ผู้เพิ่มข้อมูล"]
        ];

        <?php
        $i = 1;
        // ลูปดึงเฉพาะข้อมูลที่ถูก Filter แล้วส่งออก Excel
        foreach($all_members as $row) {
            $subgroup = json_encode($row['subgroup_title'], JSON_UNESCAPED_UNICODE);
            $wg_pos   = json_encode($row['wg_position'], JSON_UNESCAPED_UNICODE);
            $prefix   = json_encode($row['prefix'], JSON_UNESCAPED_UNICODE);
            $fname    = json_encode($row['first_name'], JSON_UNESCAPED_UNICODE);
            $lname    = json_encode($row['last_name'], JSON_UNESCAPED_UNICODE);
            $job      = json_encode($row['job_position'], JSON_UNESCAPED_UNICODE);
            $agency   = json_encode($row['agency'], JSON_UNESCAPED_UNICODE);
            $phone    = json_encode($row['phone_number'], JSON_UNESCAPED_UNICODE);
            $author   = json_encode($row['author_name'] ?? $row['username'] ?? 'System', JSON_UNESCAPED_UNICODE);

            echo "data.push([$i, $subgroup, $prefix, $fname, $lname, $job, $agency, $wg_pos, $phone, $author]);\n";
            $i++;
        }
        ?>

        let ws = XLSX.utils.aoa_to_sheet(data);
        ws['!cols'] = [
            {wch: 6}, {wch: 45}, {wch: 10}, {wch: 20}, {wch: 20}, 
            {wch: 30}, {wch: 35}, {wch: 25}, {wch: 15}, {wch: 25}
        ];

        let wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "รายชื่อคณะทำงาน");

        let dateStr = new Date().toISOString().slice(0, 10);
        XLSX.writeFile(wb, "รายชื่อคณะทำงาน_อุดรธานี_" + dateStr + ".xlsx");
    }
    </script>
</body>
</html>